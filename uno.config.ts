...
export default defineConfig({
  ...
  shortcuts: {
    ...
    'section-title':
      'm-0 font-display text-[clamp(3.2rem,7vw,6.5rem)] font-600 leading-[0.9] tracking-[-0.035em] uppercase text-[var(--color-ink)]',
    'section-copy': 'm-0 text-[var(--color-copy)] text-[1.05rem] leading-[1.7]',
    'action-button':
      'inline-flex min-h-[3.25rem] items-center justify-center gap-[0.6rem] border border-solid border-transparent px-[1.15rem] py-[0.75rem] text-[0.9rem] font-600 no-underline transition duration-180 ease-out hover:translate-y-[-2px]',
  },
  presets: [
    presetWind4({
      preflights: {
